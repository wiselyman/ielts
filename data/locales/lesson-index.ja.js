...
window.lessonIndexLocale = {
  "summaries": {
    ...
    "ted-ed-procrastination": "この TED-Ed の短い動画は、よくないと分かっていても先延ばししてしまう理由を説明します。IELTS 5.5 前後の学習者が、心理的ストレス、学習習慣、自己管理について聞く練習に向いています。重点は、先延ばし（procrastination）、扁桃体（amygdala）、神経細胞（neuron）、感情処理（emotional processing）、脅威の識別（threat identification）、ホルモン（hormone）、アドレナリン（adrenaline）、ストレス誘発性のパニック（stress-induced panic）、前頭前皮質（prefrontal cortex）、衝動（impulse）、闘争・逃走・凍結反応（fight, flight, or freeze response）、否定的感情を引き起こすこと（evoke negative feelings）、無力感（incompetence）、認識（perception）、影響を受けやすい（susceptible）、自尊心（self-esteem）、誤解（misconception）、無関心（apathy）、身体的不調（physical ailments）、ストレス反応を断ち切ること（short-circuit this stress response）、根底にある不安（underlying concerns）、自分に優しくする姿勢（self-compassion）、続かせること（perpetuate）です。",
